...
type adminConfig struct {
	...
	ReqAdminMIdKey     string
	ReqRoleIdKey       string
	ReqUsernameKey     string
	ReqNicknameKey     string
}

func (cnf adminConfig) GetAdminMId(c *gin.Context) uint {
	adminMId, ok := c.Get(cnf.ReqAdminMIdKey)
	if !ok {
		return 0
	}
	return adminMId.(uint)
}
