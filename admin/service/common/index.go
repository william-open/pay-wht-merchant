...
type IIndexService interface {
	Console(mId uint) (res map[string]interface{}, e error)
...
func (iSrv indexService) Console(mId uint) (res map[string]interface{}, e error) {
	...
	if mId > 0 {
		err = iSrv.db.Where("m_id = ?", mId).Limit(1).First(&merchant).Error
		if e = response.CheckErr(err, "Console Get Merchant err"); e != nil {
			return
		}
	}

	data, err := util.ConfigUtil.Get(iSrv.db, "website")
