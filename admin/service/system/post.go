...
type ISystemAuthPostService interface {
	All(mId uint) (res []resp.SystemAuthPostResp, e error)
...
// All 岗位所有
func (postSrv systemAuthPostService) All(mId uint) (res []resp.SystemAuthPostResp, e error) {
	var posts []system.SystemAuthPost
	err := postSrv.db.Where("m_id = ?", mId).Where("is_delete = ?", 0).Order("sort desc, id desc").Find(&posts).Error
...
func (postSrv systemAuthPostService) List(page request.PageReq, listReq req.SystemAuthPostListReq) (res response.PageResp, e error) {
	...
	// 查询
	postModel := postSrv.db.Model(&system.SystemAuthPost{}).Where("is_delete = ?", 0).Where("m_id = ?", listReq.MId)
	if listReq.Code != "" {
...
func (postSrv systemAuthPostService) Add(addReq req.SystemAuthPostAddReq) (e error) {
	r := postSrv.db.Where("m_id = ?", addReq.MId).Where("(code = ? OR name = ?) AND is_delete = ?", addReq.Code, addReq.Name, 0).Limit(1).Find(&system.SystemAuthPost{})
