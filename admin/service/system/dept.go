...
type ISystemAuthDeptService interface {
	All(mId uint) (res []resp.SystemAuthDeptResp, e error)
...
func NewSystemAuthDeptService() ISystemAuthDeptService {
	// 通过DI获取主数据库连接
	mainDB, exists := core.GetDatabase(core.DBMain)
	if !exists {
		panic("main database not initialized")
	}
	return &systemAuthDeptService{db: mainDB}
}
...
// All 部门所有
func (deptSrv systemAuthDeptService) All(mId uint) (res []resp.SystemAuthDeptResp, e error) {
	var depts []system.SystemAuthDept
	err := deptSrv.db.Where("m_id = ?", mId).Where("pid > ? AND is_delete = ?", 0, 0).Order("sort desc, id desc").Find(&depts).Error
...
func (deptSrv systemAuthDeptService) List(listReq req.SystemAuthDeptListReq) (mapList []interface{}, e error) {
	deptModel := deptSrv.db.Where("is_delete = ?", 0).Where("m_id = ?", listReq.MId)
	if listReq.Name != "" {
