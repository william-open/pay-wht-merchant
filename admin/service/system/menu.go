...
type ISystemAuthMenuService interface {
	...
	List(mId uint) (res []interface{}, e error)
...
// List 菜单列表
func (menuSrv systemAuthMenuService) List(mId uint) (res []interface{}, e error) {
	var menus []system.SystemAuthMenu
	model := menuSrv.db
	if mId > 0 {
		var menuIDs []int64

		// 第一步：获取 role_id = 1 对应的所有 menu_id
		err := menuSrv.db.
			Table("w_system_auth_perm").
			Where("role_id = ?", 1).
			Pluck("menu_id", &menuIDs).Error
		if err != nil {
			if e = response.CheckErr(err, "查询权限表失败"); e != nil {
				return
			}
		}
		model = model.Where("id IN ?", menuIDs)
	}
	err := model.Order("menu_sort desc, id").Find(&menus).Error
