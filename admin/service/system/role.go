...
type ISystemAuthRoleService interface {
	All(mId uint) (res []resp.SystemAuthRoleSimpleResp, e error)
	List(page request.PageReq, listReq req.SystemAuthRoleListReq) (res response.PageResp, e error)
...
// All 角色所有
func (roleSrv systemAuthRoleService) All(mId uint) (res []resp.SystemAuthRoleSimpleResp, e error) {
	var roles []system.SystemAuthRole
	err := roleSrv.db.Where("m_id = ?", mId).Order("sort desc, id desc").Find(&roles).Error
...
// List 根据角色ID获取菜单ID
func (roleSrv systemAuthRoleService) List(page request.PageReq, listReq req.SystemAuthRoleListReq) (res response.PageResp, e error) {
	// 分页信息
	limit := page.PageSize
	offset := page.PageSize * (page.PageNo - 1)
	// 查询 .Where("m_id = ?", listReq.MId)
	roleModel := roleSrv.db.Model(&system.SystemAuthRole{})
	if listReq.MId > 0 {
		roleModel.Where("m_id = ?", listReq.MId)
	}
	// 总数
	var count int64
	...
	// 数据
	var posts []system.SystemAuthRole
	err = roleModel.Limit(limit).Offset(offset).Order("sort desc, id desc").Find(&posts).Error
	if e = response.CheckErr(err, "List Find err"); e != nil {
		return
	}
	roleResps := []resp.SystemAuthRoleResp{}
	response.Copy(&roleResps, posts)
...
func (roleSrv systemAuthRoleService) Add(addReq req.SystemAuthRoleAddReq) (e error) {
	var role system.SystemAuthRole
	if r := roleSrv.db.Where("m_id = ?", addReq.MId).Where("name = ?", strings.Trim(addReq.Name, " ")).Limit(1).First(&role); r.RowsAffected > 0 {
...
func (roleSrv systemAuthRoleService) Edit(editReq req.SystemAuthRoleEditReq) (e error) {
	...
	var role system.SystemAuthRole
	var roleModel = roleSrv.db
	if editReq.MId > 0 {
		roleModel = roleModel.Where("m_id = ?", editReq.MId)
	}
	if r := roleModel.Where("id != ? AND name = ?", editReq.ID, strings.Trim(editReq.Name, " ")).Limit(1).First(&role); r.RowsAffected > 0 {
		return response.AssertArgumentError.Make("角色名称已存在!")
	}
	role.ID = editReq.ID
